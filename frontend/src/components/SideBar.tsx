...
export const SideBar = () => {
  ...
  return (
    <Box
      className={`flex flex-col px-2 pt-2 h-screen bg-white
    ...
    >
      ...
      <div className="mt-8 w-full flex flex-col gap-2">
        ...
        <Link
          to={ROUTES.THREADS}
          className={`flex items-center gap-3 px-2 py-2 rounded-lg hover:cursor-pointer
      ...
      <div className="mt-auto w-full relative   flex flex-row  items-center " ref={ref}>
        {secondMenuOpened && (
          ...
          >
            <Button
              variant="default"
              fullWidth
              className="div-sec-win flex w-full 
        ...
        <div
          className={` ${sideOpen ? ' -mx-2  gap-3 border-t-2 border-t-black w-[calc(100%+1rem)]' : 'w-full'}  
        ...
        >
          ...
          {sideOpen && <span className="text-sm font-medium">{userName}</span>}

          {sideOpen && <LuArrowDownUp className="ml-auto w-6 h-6 " />}
        </div>
      </div>
    </Box>
  );
